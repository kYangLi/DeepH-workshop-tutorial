...
source /data/home/scze085/.uvenv/deeph/bin/activate
...
```bash
cd ./outputs/<TIME_STAMP>
dock.error.ExposeErrorDist dft -b ../../inputs/dft -k orbital -p 16
```
...
```bash
band/target_material/
    |- POSCAR
    |- info.json
    |- overlap.h5
    |- hamiltonian.h5 -> ../../outputs/<TIME_STAMP>/dft/target_material/hamiltonian_pred.h5
```

Here, `./target_material` can be replaced with any path containing the necessary files. 
...
```bash
cd band/target_material
dock.dos.FindFermi -p 16 .
```
...
``` bash
cd band/target_material
dock.band.CalcData -p 16 .
```

The `-p 16` specifies 16-core parallelization over k points and can be adjusted according to available computational resources.
